...
import { Progress } from "@/apps/gc/components/ui/progress";
import { usePortfolioAnalytics, useRecentPayApplications, useProjectHealthScores } from "@/apps/gc/hooks/usePortfolioAnalytics";
            ...
            <div className="space-y-3 max-h-[300px] overflow-y-auto">
              {!payAppsLoading && recentPayApps && recentPayApps.length > 0 ? (
                recentPayApps.map((pa) => (
                  <Link
                    key={pa.id}
                    to={`/gc-dashboard/projects/${pa.project_id}?tab=budget`}
          ...
          <CardContent>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {healthScores.map((project) => (
                <Link
                  key={project.id}
                  to={`/gc-dashboard/projects/${project.id}`}

        ...
        <div className="flex-1 w-full bg-white dark:bg-[#0f1115] text-gray-900 dark:text-white pb-20">
            <div className="bg-gray-50/50 dark:bg-[#1c1e24]/50 border-b border-gray-200 dark:border-white/5 px-8 py-10 mb-8">
                <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    ...
                    <Button className="bg-black dark:bg-yellow-500 text-white dark:text-black font-black uppercase text-xs tracking-widest rounded-xl px-8 h-12 shadow-xl hover:bg-yellow-600 transition-all">
            ...
            <div className="max-w-7xl mx-auto px-8">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                    {[
                        { label: 'Active Bids', value: '18', icon: Clock, color: 'text-yellow-600 dark:text-yellow-400' },
                        { label: 'Under Review', value: '05', icon: Filter, color: 'text-gray-900 dark:text-white' },
                        { label: 'Total Value', value: '$3.4M', icon: DollarSign, color: 'text-yellow-600 dark:text-yellow-400' }
                    ].map(stat => (
                            ...
                            {['active', 'awarded', 'closed'].map(stage => (
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        activeStage === stage ? "bg-white dark:bg-yellow-500 text-black shadow-md" : "text-gray-400 hover:text-gray-900 dark:hover:text-white"
                                    )}
                                >
                                    {stage}
                                </button>
                                ...
                                {bids.map(bid => (
                                    <tr key={bid.id} className="group border-b border-gray-50 dark:border-white/5 hover:bg-gray-50/50 dark:hover:bg-white/[0.02] transition-colors">
                                        <td className="px-8 py-6">
                                            <div className="flex items-center gap-4">
                                                <div className="w-12 h-12 rounded-xl bg-gray-100 dark:bg-black/40 flex items-center justify-center text-yellow-500 font-black text-xs">
                                                    {bid.id.split('-')[1]}
                                                </div>
                                                <div>
                                                    <p className="font-black text-gray-900 dark:text-white">{bid.project}</p>
                                                    <p className="text-xs font-bold text-gray-400 group-hover:text-yellow-600 transition-colors">{bid.contractor}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-8 py-6">
                                        ...
                                        <td className="px-8 py-6 text-right">
                                            <Button variant="ghost" className="rounded-xl h-10 w-10 p-0 hover:bg-yellow-400 hover:text-black">
